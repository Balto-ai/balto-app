...
export default function DogSearchPage() {
  ...
  const [selectedDistance, setSelectedDistance] = React.useState('') // integer, or false if distance isn't selected
  const [selectedShelters,  setSelectedShelters] = React.useState([]) // selected shelters by their Id, ex. [3, 4]

  console.log(111, selectedDistance)
  ...
  const handleCheck = (evt, stateArr, setStateArr) => {
    let newArr = [...stateArr]
    if (evt.target.checked) { // checking if the checkbox just got checked or unchecked
    ...
    <div className="filter-sidebar">
      ...
      <Accordion alwaysOpen>
        ...
        <Accordion.Item eventKey="breed">
          <Accordion.Header>
            Breed
            {selectedBreeds.length > 0 ? <Badge pill bg="primary">{selectedBreeds.length}</Badge> : null}
          </Accordion.Header>
          <Accordion.Body>
        ...
        <Accordion.Item eventKey="size">
          <Accordion.Header>
            Size
            {selectedSizes.length > 0 ? <Badge pill bg="primary">{selectedSizes.length}</Badge> : null}
          </Accordion.Header>
          <Accordion.Body>
            <Form>
              {sizeOptions.map((option) => (
                <div key={option} className="mb-3">
                  <Form.Check type='checkbox'
                    ...
                    label={option}
                    value={option}
                    onChange={(evt)=>{handleCheck(evt, selectedSizes, setSelectedSizes)}} />
                </div>
              ))}
            </Form>
          </Accordion.Body>
        ...
        <Accordion.Item eventKey="gender">
          <Accordion.Header>
            Gender
            {selectedGenders.length > 0 ? <Badge pill bg="primary">{selectedGenders.length}</Badge> : null}
          </Accordion.Header>
          <Accordion.Body>
            <Form>
              {genderOptions.map((option) => (
                <div key={option} className="mb-3">
                  <Form.Check type='checkbox'
                    ...
                    label={option}
                    value={option}
                    onChange={(evt)=>{handleCheck(evt, selectedGenders, setSelectedGenders)}} />
                </div>
              ))}
            </Form>
          </Accordion.Body>
        ...
        <Accordion.Item eventKey="good-with">
          <Accordion.Header>
            Good With
            {selectedGoodWith.length > 0 ? <Badge pill bg="primary">{selectedGoodWith.length}</Badge> : null}
          </Accordion.Header>
          <Accordion.Body>
            <Form>
              {goodWithOptions.map((option) => (
                <div key={option} className="mb-3">
                  ...
                  label={option}
                  value={option}
                  onChange={(evt)=>{handleCheck(evt, selectedGoodWith, setSelectedGoodWith)}} />
                </div>
              ))}
            </Form>
          </Accordion.Body>
        ...
        <Accordion.Item eventKey="distance">
          <Accordion.Header>
            Distance
            {selectedDistance && <Badge pill bg="primary">1</Badge>}
          </Accordion.Header>
          <Accordion.Body>
            <Form>
            <Form.Select
              ...
              onChange={(evt) => { setSelectedDistance(evt.target.value) }} >
        ...
        <Accordion.Item eventKey="shelter">
          <Accordion.Header>
            Shelter
            {selectedShelters.length > 0 ? <Badge pill bg="primary">{selectedShelters.length}</Badge> : null}
            </Accordion.Header>
          <Accordion.Body>
